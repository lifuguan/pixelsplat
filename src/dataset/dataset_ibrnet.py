...
class IBRLLFFDataset(IterableDataset):
    def __init__(self, args, mode, step_tacker, **kwargs):
        ...
        all_scenes = os.listdir(self.folder_path) + glob.glob(self.folder_path1 + '*') + glob.glob(self.folder_path2 + '*')
        ...
        for i, scene in enumerate(self.scenes):
            factor = 2 if 'ibrnet_collected_1'  in scene else 8
            
            if 'ibrnet_collected_1' in scene or 'ibrnet_collected_2' in scene:
                scene_path = scene
            else:
    ...
    def __iter__(self):
        org_idx = list(range(len(self.render_rgb_files)))
        random.shuffle(org_idx)
        for idx in org_idx:
